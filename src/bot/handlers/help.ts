...
/start ${fmt.escape("or")} ${fmt.code("tg start [claude|codex|pi|kimi] [project|args]")} ${fmt.escape("—")} Start from Camp, or restart an attached wrapper session
/kill ${fmt.escape("or")} ${fmt.code("tg kill")} ${fmt.escape("—")} Kill the current chat session process
