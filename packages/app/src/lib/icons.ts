...
/** Return the appropriate icon SVG string for a channel address */
export function channelIcon(channel: string | null | undefined): string | null {
  if (!channel) return null;
  const ch = channel.toLowerCase();
  if (ch.includes('telegram')) return telegramIcon;
  return null;
}

/** Format a prefixed channel string for display, e.g. "telegram:Dev2" → "Telegram: Dev2" */
export function channelLabel(channel: string | null | undefined): string | null {
  if (!channel) return null;
  const idx = channel.indexOf(':');
  if (idx >= 0) {
    const type = channel.slice(0, idx);
    const name = channel.slice(idx + 1);
