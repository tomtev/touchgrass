<script lang="ts">
  import { decodeDNA, generateGrid, EYES, MOUTHS, HATS, BODIES, LEGS } from '@touchgrass/avatar';
  import type { Pixel, DecodedDNA } from '@touchgrass/avatar';
  ...
  const traits = $derived(
    dna ? decodeDNA(dna) : traitsFromHash(hashName(name))
  );
