...
2. **`agents/`** holds agent definitions as simple markdown files with YAML frontmatter
...
One `agent.md` file is the single source of truth per agent. The sync script outputs the right config format for each CLI tool.
